<div align=center>

# Challenges write-ups

<!-- https://shields.io/badges -->
[![THM](https://img.shields.io/badge/TryHackMe-151C2B?style=flat&logo=tryhackme&logoColor=A20606 "THM")](https://tryhackme.com/p/pedrofparmentiere)
[![OTW](https://img.shields.io/badge/OverTheWire-333?style=flat "OTW")](https://overthewire.org/wargames/)
[![PicoCTF](https://img.shields.io/badge/PicoCTF-C41230?style=flat "PicoCTF")](https://play.picoctf.org/users/pfpe20)
![HTB](https://img.shields.io/badge/HackTheBox-0B121F?style=flat&logo=hackthebox&logoColor=9FEF00 "HTB")

*My personal collection of CTF write-ups and challenge solutions*

</div>

---

## 🔎 About

### This repository

This repository provides solutions to CTFs (*Capture the flag*) from various platforms. It covers exploitation techniques, vulnerabilities, tooling and methodology. Intended as both a personal reference and public resource.

**Environment:** The default OS you will find in these challenges is **Kali Linux**
<img src="https://cdn.simpleicons.org/kalilinux/557C94" width="24" height="24" alt="Kali Linux" style="vertical-align: middle;" />

**Tools:** See my [Tool Index](https://github.com/PFPE20/find_commands) - updated regularly

---

## 🎯 Write-ups

### TryHackMe

| Name | Difficulty | Type | Write-up |
| ---- | ---------- | ---- | -------- |
| Billing | Easy | RCE, CVE-2023-30258 | [Writeup](./tryhackme/Billing-writeup/BILLING.md) |
| IDE | Easy | RCE | [Writeup](./tryhackme/IDE-writeup/IDE.md) |
| The Sticker Shop | Easy | Stored XSS, CVE-2024-34069 | [Writeup](./tryhackme/TheStickerShopping-writeup/TSS.md) |
| Library | Easy | Credentials Bruteforcing, Script Hijacking | [Writeup](./tryhackme/Library-writeup/LIBRARY.md) |
| Thompson | Easy | Default Credentials, Malicious WAR File Upload → RCE | [Writeup](./tryhackme/Thompson-writeup/THOMPSON.md) |
| Valley | Easy | PCAP Analysis, Python Library Hijacking | [Writeup](./tryhackme/Valley-writeup/VALLEY.md) |

### OverTheWire

| Name | Difficulty | Type | Write-up |
| ---- | ---------- | ---- | -------- |
| Leviathan | Easy | UNIX based commands | [Writeup](./overthewire/LEVIATHAN.md) |

### PicoCTF

> *Coming soon*

### Hack The Box

> *Coming soon*